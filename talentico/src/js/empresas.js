...
function render() {
  ...
  companies.forEach((company) => {
    ...
    row.innerHTML = `
      ...
      <td><button type="button" data-action="edit">Editar</button><button type="button" data-action="delete">Eliminar</button></td>`;
...
async function loadCompanies() {
  try {
    const data = await apiFetch('/carts', {}, DUMMY_JSON_API_URL);
    companies = data.carts || [];
    render();
  } catch (error) {
...
form.addEventListener('submit', async (event) => {
  ...
  const payload = { userId, products: [{ id: 1, quantity: totalProducts }] };
  try {
    const endpoint = id ? `/carts/${id}` : '/carts/add';
    const result = await apiFetch(endpoint, { method: id ? 'PUT' : 'POST', body: JSON.stringify(payload) }, DUMMY_JSON_API_URL);
...
modalConfirm.addEventListener('click', async () => {
  try {
    await apiFetch(`/carts/${companyToDelete}`, { method: 'DELETE' }, DUMMY_JSON_API_URL);
